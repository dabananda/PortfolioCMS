...
function ProjectModal({
  ...
  if (!open) return null;
  const inputCls =
    "w-full px-3.5 py-2.5 rounded-lg bg-white/4 border border-white/8 text-sm text-white/90 placeholder-white/25 focus:outline-none focus:border-violet-500/50 focus:bg-white/6 transition-all";
      ...
      <div className="bg-[#161b22] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <div
              className="relative h-36 rounded-xl overflow-hidden bg-white/3 border border-white/8 cursor-pointer hover:border-violet-500/40 transition-colors group"
            ...
            <input
              ...
              className="hidden"
              onChange={(e) =>
                e.target.files?.[0] && handleImgUpload(e.target.files[0])
              }
            />
          </div>
            ...
            {form.technologies.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {form.technologies.map((t, i) => (
                  <span
                    key={t}
                    className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border ${TECH_COLORS[i % TECH_COLORS.length]}`}
                  >
                    {t}
                    <button
                      onClick={() =>
    ...
    >
      <div className="bg-[#161b22] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded-lg text-sm text-white/50 hover:text-white/80 bg-white/5 hover:bg-white/8 transition-all"
          >
            Cancel
          </button>
          <button
...
function ProjectsPageContent() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      fetchWithAuthMutation(`project/${id}`, "DELETE"),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["projects"] });
      qc.invalidateQueries({ queryKey: ["projects-count"] });
    ...
    onError: () =>
      setToast({ type: "error", msg: "Failed to delete project." }),
  });
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-[#161b22] border border-white/6 rounded-xl overflow-hidden animate-pulse"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.map((project, i) => (
            <div
              key={project.id}
              className="bg-[#161b22] border border-white/6 rounded-xl overflow-hidden hover:border-white/12 transition-colors group flex flex-col"
            >
              {/* Image */}
              <div className="relative h-40 shrink-0">
                {project.imageUrl ? (
                  ...
                  <div
                    className={`w-full h-full bg-gradient-to-br ${CARD_GRADIENTS[i % CARD_GRADIENTS.length]} opacity-70`}
                  />
                )}
              ...
              <div className="p-4 flex flex-col flex-1">
                <h3 className="font-semibold text-white/90 leading-snug mb-1.5">
                  {project.title}
                </h3>
                ...
                {project.technologies.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {project.technologies.slice(0, 4).map((t, ti) => (
                      <span
                        key={t}
                        className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium border ${TECH_COLORS[ti % TECH_COLORS.length]}`}
                      >
                        <Tag size={8} /> {t}
                      </span>
                    ))}
                    {project.technologies.length > 4 && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-medium border border-white/10 text-white/30">
                        +{project.technologies.length - 4}
                      </span>
                    )}
                  </div>
